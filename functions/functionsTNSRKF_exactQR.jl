module functionsTNSRKF_exactQR
...
export TNSRKF, predictions
    ...
    function meanupdate(yt,Φt,mtt0,mt,LΦS,Lt,maxiter)
        # the norm core dd is the one in Lt that has the column index
    ...
    function covupdate(initcov,initcov_,dd,Φt,priorcov,maxiter,σ²,t,invSt,LΦS)
        ...
        for iter = 1:maxiter
            for k = 1:2D-2
                ...
                Ltt_1   = copyTT(Ltt2)
                Ltt_1[dd]= Ltt_1[dd][:,:,1:Int(size(Ltt_1[dd],3)/2),:]
                Qd      = getttm(ttm2ttv(Ltt_1),d)
                Term1   = reshape(transpose(Qd)*ttm2ttv(Lt),size(Ltt_1[d]))
                Term2   = term2(Φttm,Lt,Ltt_1,invSt,d)

                Ltt_2   = copyTT(Ltt2)
                Ltt_2[dd]= Ltt_2[dd][:,:,Int(size(Ltt_2[dd],3)/2)+1:end,:]
                if t == 1
                    Term3   = term3(Ltt_2,priorcov,priorcov,LΦσS,d)
                else
                    Term3   = term3(Ltt_2,initcov_,priorcov,LΦσS,d)
                end
                
                if d == dd
                    Ltt2[d] = cat(Term1-Term2,Term3,dims=3)
                else
                    Ltt2[d] = Term1-Term2+Term3
                end

                Ltt2    = ttv2ttm(shiftTTnorm(ttm2ttv(Ltt2),d,Dir[k]),size(Ltt2))
            end
        end
    ...
    function TNSRKF(N,y,Φ,rnks_mean,rnks_cova,M,dd,P0,maxiter,σ²,p)
        # computes mean update with ALS and covariance root with ALS
        # the amount of times that the columns can be doubled
        ...
        for t = 1:N
            ...
            if t == 1
                ...
                LΦS,invSt   = getKalmangain(Φt,initcov_,σ²)
                mtt,res     = meanupdate(y[t],Φt,mtt0,mt,LΦS,initcov_,maxiter)
            end
            ...
            # update covariance
            Lt,Ltt2 = covupdate(initcov,initcov_,dd,Φt,priorcov,maxiter,σ²,t,invSt,LΦS);

            mt_all[t]  = mtt
            Lt_all[t]  = Ltt2

            # for next iteration: check if columns need to be doubled again
            Lt      = copyTT(Ltt2)
            Ltdd    = Lt[dd]
            sz      = size(Lt[dd])
            Ltdd    = reshape(permutedims(Ltdd,[1,2,4,3]),(sz[1]*sz[2]*sz[4],sz[3]))
            F       = svd(Ltdd)

            if size(Ltt2[dd],3)/2+1 < length(F.S)
                sv_cut = F.S[Int(size(Ltt2[dd],3)/2+1)]
            else
                sv_cut = 0
            end

            if sv_cut > eps(10.) && p_ < p
                # double columns for prior and initialization for next time step
                priorcov    = copyTT(Ltt2)
                initcov_    = copyTT(Ltt2)
                initcov     = copyTT(Ltt2)
                initcov[dd] = randn(size(Ltt2[dd],1),size(Ltt2[dd],2),2*size(Ltt2[dd],3),size(Ltt2[dd],4))
                p_ = p_ + 1
            else
                # make QR step to cut columns in half
                Ltdd    = F.U[:,1:Int(size(Ltt2[dd],3)/2)]*diagm(F.S[1:Int(size(Ltt2[dd],3)/2)])
                Ltdd    = reshape(Ltdd,sz[1],sz[2],sz[4],Int(size(Ltt2[dd],3)/2))
                Lt[dd]  = permutedims(Ltdd,[1,2,4,3])
    ...
    function predictions(N,Nstar,Φs,mt_all,Lt_all,ytest)
        ...
        for nstar = 1:Nstar
            ...
            for t = 1:N
                ...
                σstar[nstar,t]  = (ΦsiL*transpose(ΦsiL))[1]
            end
        end
        println("done")
        ...
        return rootmse,nll,mstar,sqrt.(σstar)
    end    
